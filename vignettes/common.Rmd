...
Here is a list of the functions and a short explanation of each:{#top}
...
### File searching capability {#find}
Sometimes you know the file name of the file you are looking for, but 
do not know the exact location.  It might be in the directory above your
program, or it might be in the directory below.  The `file.find()`
...
and it will begin looking in the base directory.  If the file is not 
found in the base directory, it will expand the search above and below the 
base directory.  The search routine will continue expanding the search
until it finds a file that matchs the search criteria, or hits the 
limits imposed by the `up` and `down` parameters.  Here is an example:
```{r eval=FALSE, echo=TRUE}
# Look for a file named "globals.R"
pths <- file.find(Sys.path(), "globals.R")
pths
```
