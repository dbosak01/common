...
to run an entire directory full of programs?  Base R does not have 
a built-in way to do this.  The `source.all()` function
fills this need.  

The function allows you to run many programs in batch. 
You can point the function to a directory, and run all the programs in that 
directory at once.  Conveniently, the function
